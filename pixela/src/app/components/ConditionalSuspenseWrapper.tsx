...
const CONFIG = {
  ...
  MINIMAL_FALLBACK_STYLES:
    "relative w-full min-h-[80vh] sm:min-h-[85vh] md:min-h-screen lg:h-screen 2k:h-[70vh] bg-pixela-dark",
} as const;
 ...
 */
export const ConditionalSuspenseWrapper = ({
  children,
}: ConditionalSuspenseWrapperProps) => {
  const [showMinimalFallback, setShowMinimalFallback] = useState(true);

  useEffect(() => {
    // Logic inside useEffect to avoid side-effects during render (strict mode safe)
    const isFirstVisit = detectFirstVisit();
    setShowMinimalFallback(isFirstVisit);
  }, []);
