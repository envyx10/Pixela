...
export const tmdbImageHelpers = {
   ...
   */
  poster: (path: string | null | undefined, size = DEFAULT_IMAGE_SIZES.POSTER) => 
    buildTmdbImageUrl(path, size),
  
  /**
   * Construye URL para backdrop (fondo)
   */
  backdrop: (path: string | null | undefined, size = DEFAULT_IMAGE_SIZES.BACKDROP) => 
    buildTmdbImageUrl(path, size),
  
  /**
   * Construye URL para perfil de persona
   */
  profile: (path: string | null | undefined, size = DEFAULT_IMAGE_SIZES.PROFILE) => 
    buildTmdbImageUrl(path, size),
} as const;
...
export const TMDB_PLACEHOLDER = {
  ...
  BACKDROP: 'https://via.placeholder.com/1280x720/1a1a1a/666666?text=Sin+Imagen',
